...
def signup():
    ...
    for i in accounts:
        while mail == i:
            goback = input(
                "\nGirilen Mail Adresi Halihazırda Kayıtlıdır, Kayıt Olmaya Devam Etmek İçin 'E', Programa Geri Dönmek İçin 'Q' Tuşlayınız: "
            ).upper()
            if goback == "E":
    ...
    while any(new_user == v[0] for v in accounts.values()):
        print(
            "Girilen Kullanıcı Adı Halihazırda Kullanımdadır, Lütfen Farklı Bir Kullanıcı Adı Seçiniz!"
        )
        new_user = input("Kullanıcı Adı: ")
    ...
    print(
        f"Sayın Kullanıcı {new_user}, Kaydınız Başarıyla Gerçekleşmiştir!\nPrograma Yönlendiriliyorsunuz..."
    )
    ...
    while True:
        ...
        for v in accounts.values():
            if username == v[0] and password == v[1]:
                ...
                ask_signup = input(
                    "Kayıt Olmak İçin 'E' Tekrar Denemek İçin 'H' Tuşlayınız: "
                ).upper()
                if ask_signup == "E":
                    return "Signup"
                elif ask_signup == "H":
...
def changepass():
    ...
    if mail in accounts:
        ...
        while trys < 3:
            verify = int(
                input(f"{mail} Adresine Gelen 6 Haneli Doğrulama Kodunu Giriniz: ")
            )
            if verify == vcode:
        ...
        ask_signup = input(
            "Kayıt Olmak İçin 'E', Sisteme Geri Dönmek İçin 'Q' Tuşlayınız (E/Q): "
        ).upper()
        if ask_signup == "E":
            return "Noaccount"
        else:
